...
$compraP = new Model\CompraProducto();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    switch ($action) {
        case 'update':
            ...
            break;
        case 'create':
            $compraP->setIdCom($data->idCom);
            $compraP->setIdPro($data->idPro);
            $compraP->setCantidadProCom($data->cantidad);
            $compraP->setUsuarioCreacion($idUser);
            $compraP->setUsuarioModificacion($idUser);
            $result = $compraP->create();
            $response = array(
                'result' => $result
            );
            break;
